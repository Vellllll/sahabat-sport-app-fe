    ...
    <form ref={formRef} action={formAction} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-4 focus:ring-brand/10 focus:border-brand outline-none transition-all placeholder:text-slate-300 font-medium text-slate-700"
        />
        {state.errors?.name && (
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <select
            name="categoryId"
            id="categoryId"
            className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-4 focus:ring-brand/10 focus:border-brand outline-none transition-all text-slate-600 font-medium appearance-none cursor-pointer"
          >
      ...
      <div className="pt-4 space-y-4">
        {state.message && (
          <div className={`text-center py-2 rounded-lg text-xs font-bold tracking-wide uppercase ${state.errors && Object.keys(state.errors).length > 0 ? 'bg-red-50 text-red-500' : 'bg-blue-50 text-brand'}`}>
            {state.message}
          </div>
        )}
        ...
        <button
          type="submit"
          disabled={isPending}
          className="w-full bg-brand text-white py-4 rounded-xl font-bold text-sm shadow-lg shadow-brand/20 hover:bg-brand-hover hover:shadow-xl hover:shadow-brand/30 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
        >
