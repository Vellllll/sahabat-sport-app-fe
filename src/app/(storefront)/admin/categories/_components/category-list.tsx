    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex gap-2">
                ...
                <button onClick={() => updateUrl({ q: inputValue, page: 1 })} disabled={isPending} className="bg-brand text-white px-6 rounded-2xl font-bold text-xs hover:bg-brand-hover transition-all disabled:opacity-50 flex items-center gap-2">
            ...
            <div className={`space-y-2 min-h-[300px] ${isPending ? 'opacity-50' : 'opacity-100 transition-opacity'}`}>
                {initialData.map((cat) => (
                    <div key={cat.id} className="group flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl hover:border-brand/30 hover:shadow-sm transition-all">
                        <Link href={`/admin/categories/${cat.id}`} className="flex items-center gap-4 min-w-0 flex-1">
                            <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:text-brand group-hover:bg-brand/5 transition-colors shrink-0">
