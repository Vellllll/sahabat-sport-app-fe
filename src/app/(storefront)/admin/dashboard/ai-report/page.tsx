        ...
        <main className="min-h-screen bg-[#F8FAFC] py-8 px-4 sm:px-6 lg:px-8 flex flex-col justify-between max-w-6xl mx-auto space-y-6">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-200/60 shrink-0">
                <div className="space-y-1">
                    <Link
                        ...
                        className="text-xs font-black uppercase tracking-widest text-slate-400 hover:text-brand flex items-center gap-1.5 transition-colors mb-1"
                    >
                    ...
                    <h1 className="text-xl font-black text-slate-900 uppercase flex items-center gap-2">
                        <Sparkles className="h-5 w-5 text-brand" /> Ask Database Agent AI
                    </h1>
                </div>
            ...
            <div className="flex-1 bg-white border border-slate-100 rounded-[32px] shadow-sm overflow-y-auto p-4 sm:p-6 h-[calc(100vh-280px)] space-y-6 scrollbar-thin">
              ...
              `}>
                  ...
                  ${isBot
                                        ...
                                        : 'bg-brand text-white border-brand-hover rounded-tr-none'
                                    }
                `}>
                ...
                {isLoading && (
                    <div className="flex gap-4 max-w-[80%] mr-auto animate-in fade-in duration-200">
                        <div className="h-9 w-9 rounded-xl bg-blue-50 text-brand border border-blue-100 flex items-center justify-center shrink-0 shadow-sm animate-pulse">
            ...
            <form onSubmit={handleSendMessage} className="bg-white p-2.5 rounded-2xl border border-slate-100 shadow-md flex items-center gap-2 shrink-0">
                ...
                <button
                    ...
                    className="h-11 w-11 bg-brand text-white rounded-xl flex items-center justify-center transition-all shadow-md shadow-brand/10 hover:bg-brand-hover active:scale-95 disabled:opacity-30 disabled:scale-100 cursor-pointer"
                >
            ...
            <div className="hidden sm:flex items-center justify-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-wider select-none shrink-0">
                <ClipboardList className="h-3.5 w-3.5 text-brand" /> Rekomendasi Pertanyaan:
                <button
                    onClick={() => setInputMessage("Berapa omset kotor toko kita sepanjang tahun 2026?")}
                    disabled={isLoading}
                    className="px-2.5 py-1 bg-white border border-slate-100 rounded-lg text-slate-500 hover:border-brand hover:text-brand transition-colors cursor-pointer disabled:opacity-50"
                >
                    Omset 2026
                </button>
                <button
