...
export function AddToCartButton({ productItemId, productName = "Item", productPrice = 0 }: Props) {
  ...
  return (
    <button
      ...
      disabled={isPending}
      className="w-full h-12 bg-brand hover:bg-brand-hover disabled:bg-slate-100 text-white disabled:text-slate-400 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-md shadow-brand/10 active:scale-[0.99] flex items-center justify-center gap-2.5 cursor-pointer disabled:cursor-not-allowed"
    >
      {isPending ? (
        <>
