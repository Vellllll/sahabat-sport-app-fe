...
import { Plus, Edit2, Trash2, Box, DollarSign, Eye, EyeOff, Image as ImageIcon, Loader2 } from 'lucide-react';
...
export default function ProductItemManager({ productId, initialItems }: Props) {
  ...
  const handleSuccessModal = () => {
    ...
    setTimeout(() => setEditingItem(null), 300); // Bersihkan state setelah animasi tutup
    startTransition(() => {
      router.refresh();
    });
  };
    ...
    <div className="space-y-6 relative">
      ...
      {isPending && (
        <div className="absolute inset-0 bg-white/50 backdrop-blur-[1px] z-10 flex items-center justify-center rounded-[32px]">
          <Loader2 className="h-8 w-8 animate-spin text-[#165dfc]" />
        </div>
      )}
      ...
      <div className="flex items-center justify-between px-2">
        ...
        <button
          ...
          className="bg-[#165dfc] text-white px-5 py-2.5 rounded-xl font-bold text-xs tracking-widest hover:bg-[#124ecb] shadow-lg transition-all flex items-center gap-2"
        >
      ...
      {isModalOpen && (
        <ItemFormModal
          key={editingItem ? editingItem.id : 'new-item'} // KUNCI UTAMA ANTI BUG
          productId={productId}
            ...
            {initialItems.map((item) => (
              <div key={item.id} className="group flex flex-col sm:flex-row sm:items-center justify-between p-4 hover:bg-slate-50 rounded-2xl transition-colors border border-transparent hover:border-slate-100 gap-4">
                ...
                <div className="flex items-center gap-1 sm:opacity-0 group-hover:opacity-100 transition-opacity pl-4 border-l border-slate-100 shrink-0">
                  <button onClick={() => handleOpenEdit(item)} className="p-2.5 text-slate-400 hover:text-[#165dfc] hover:bg-[#165dfc]/5 rounded-xl transition-all">
