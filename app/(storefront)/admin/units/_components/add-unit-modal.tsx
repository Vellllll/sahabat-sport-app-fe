...
export default function AddUnitModal() {
  ...
  return (
    <>
      <button onClick={() => setIsOpen(true)} className="bg-[#165dfc] text-white px-5 py-3 rounded-2xl font-bold text-xs tracking-widest hover:bg-[#124ecb] shadow-lg shadow-[#165dfc]/20 transition-all flex items-center gap-2">
            ...
            <form ref={formRef} action={formAction} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input name="name" required disabled={isPending} placeholder="Contoh: Kilogram" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-transparent focus:bg-white focus:border-[#165dfc] rounded-xl outline-none font-medium text-slate-700" />
                </div>
              ...
              <div className="space-y-2">
                ...
                <input type="number" name="quantity" required disabled={isPending} placeholder="Contoh: 250" min="1" className="w-full px-4 py-3 bg-slate-50 border border-transparent focus:bg-white focus:border-[#165dfc] rounded-xl outline-none font-medium text-slate-700" />
                {state.errors?.quantity && <p className="text-red-500 text-[10px] font-bold ml-1">{state.errors.quantity[0]}</p>}
              </div>

              <button type="submit" disabled={isPending} className="w-full bg-[#165dfc] text-white py-4 rounded-2xl font-bold text-xs tracking-widest hover:bg-[#124ecb] transition-all flex items-center justify-center disabled:opacity-50 uppercase">
