...
import { useCart } from '@/context/cart-context'; // 🟢 1. IMPORT GLOBAL CART CONTEXT HOOK
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function CartDisplayGrid({ initialItems }: Props) {
  ...
  // 🟢 2. INJEKSI FUNGSI CONTEXT MUTATION LIVE
  ...
  const handleQuantityChange = (
    ...
    startTransition(async () => {
      // 1. Tembak mutasi penambahan ke backend NestJS via Server Action
      const result = await updateCartItemQuantity(productItemId, incrementValue);
      
      if (!result.success) {
        toast.error(result.error);
      } else {
        // 2. Update state visual lokal halaman keranjang belanja
        ...
        // 🟢 3. SINKRONISASI DUA ARAH: Update State Global Context Klien
        const targetItemData = items.find(i => i.product_item.id === productItemId);
        if (targetItemData) {
          if (direction === 'plus') {
            // Jika menekan (+), panggil addToCart dengan kuantitas 1 untuk menaikkan counter navbar
            addToCart({
              id: productItemId.toString(),
              name: targetItemData.product_item.name,
              price: Number(targetItemData.product_item.price)
            }, 1); // 🌟 Mengalirkan angka 1 secara dinamis sesuai perbaikan fungsi kita sebelumnya
          } else {
            // Jika menekan (-), panggil fungsi decreaseQuantity dari context
  ...
  const confirmDeleteAction = () => {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        ...
        // 🟢 4. INTEGRASI: Hapus total dari global context agar badge navbar langsung berkurang drastis
          ...
          return (
            <div key={item.id} className="py-6 flex items-start gap-4 sm:gap-6 first:pt-0 last:pb-0">
              <div className="relative h-20 w-20 bg-slate-50 rounded-2xl overflow-hidden border border-slate-100/70 shrink-0 flex items-center justify-center">
                <img
                  src={item.product_item.pic_url || '/public/uploads/default-product.jpg'}
                ...
                <div className="flex items-center gap-3 pt-3">
                  <div className="flex items-center border rounded-lg p-0.5 transition-colors bg-slate-50 border-slate-100">
                    <button
                      ...
                      className="w-7 h-7 font-bold text-slate-500 hover:bg-white hover:text-red-500 rounded-md transition-all text-xs cursor-pointer"
                    >
                      -
                    </button>
                    ...
                    <button
                      ...
                      className="w-7 h-7 font-bold text-slate-500 hover:bg-white hover:text-slate-800 rounded-md transition-all text-xs cursor-pointer"
                    >
                      +
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="p-1.5 text-slate-300 hover:text-red-500 rounded-lg transition-colors cursor-pointer disabled:opacity-30"
                  >
